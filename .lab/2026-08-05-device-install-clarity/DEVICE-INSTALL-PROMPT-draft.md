# DEVICE-INSTALL-PROMPT (lab draft)
...
1. **Detectar OS**
   ...
   - Linux / macOS / WSL → `tooling/scripts/orchestrator.sh` (o wrapper `tooling/scripts/orchestrator` si existe)
   - No uses `curl | bash`. No embebas PAT ni tokens.

2. **Obtener el pack**
   - Si ya estás en la raíz del repo clonado, usá `--source local`.
...
3. **Preguntar al humano**
   ...
   - Para prueba segura sin tocar otros repos: usá el sandbox del pack `tooling/sandbox/pilot`.
...
- Tras APPROVE, copiar a `docs/agent/DEVICE-INSTALL-PROMPT.md` (prod).
- Ajustar digest SHA256 del release en FIRST-RUN / TEAM-SHARE post v1.2.0.
