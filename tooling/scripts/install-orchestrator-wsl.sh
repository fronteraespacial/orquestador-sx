#!/usr/bin/env bash
# SpaceX Orchestrator — install-orchestrator-wsl.sh
...
usage() {
  cat <<'EOF'
Usage: install-orchestrator-wsl.sh [OPTIONS]

Safe installer for SpaceX Orchestrator templates.
...
# Explicit empty init — required for `set -u` when counts stay 0
ENTRIES=()
COPIED=()
REFRESHED=()
SKIPPED=()
