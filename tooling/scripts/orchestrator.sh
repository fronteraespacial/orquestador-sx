#!/usr/bin/env bash
# SpaceX Orchestrator — orchestrator.sh (WSL/Linux)
...
Commands:
  ...
  update     --check (gh release, 24h throttle) | --apply (verify SHA256, reinstall)
...
local_pack_sha256() {
  sha256sum "$PACK_ROOT/VERSION" | awk '{print $1}'
}
...
cmd_init() {
  ...
  install_scope="$SCOPE"
  local yes_flag=""
  [[ "$CONFIRM_USER" == true ]] && yes_flag="--yes"
...
cmd_update_check() {
  ...
  remote="$(gh release view --repo "$DEFAULT_REPO" --json tagName -q .tagName | sed 's/^v//')"
  save_check_ts "$root"
...
cmd_update_apply() {
  ...
  tag="$(gh release view --repo "$DEFAULT_REPO" --json tagName -q .tagName)"
  ver="${tag#v}"
  ...
  log "Downloading $tag..."
  (cd "$work_dir" && gh release download "$tag" --repo "$DEFAULT_REPO" --pattern 'SHA256SUMS' --pattern '*.zip')
  verify_sha256sums "$work_dir"
  ...
  bash "$extracted/tooling/scripts/install-orchestrator-wsl.sh" --scope project --target "$root" --yes
  local zip_hash
